...
func (s *TLSSuite) SetUpTest(c *check.C) {
	...
	testAuthServer, err := NewAuthServer(AuthServerConfig{
		Dir:        s.dataDir,
		Clock:      s.clock,
		PrivateKey: fixtures.PEMBytes["rsa"],
	})
	c.Assert(err, check.IsNil)
	s.server, err = testAuthServer.NewTLSServer()
	c.Assert(err, check.IsNil)
}
...
func (s *TLSSuite) TestRemoteBuiltinRole(c *check.C) {
	remoteServer, err := NewAuthServer(AuthServerConfig{
		Dir:         c.MkDir(),
		ClusterName: "remote",
		Clock:       s.clock,
	})
	...
	remoteProxy, err := remoteServer.NewRemoteClient(
		Builtin(teleport.RoleProxy), s.server.Addr(), certPool)
	c.Assert(err, check.IsNil)

	// certificate authority is not recognized, because
	// the trust has not been established yet
	_, err = remoteProxy.GetNodes(defaults.Namespace, resource.SkipValidation())
	...
	remoteAuth, err := remoteServer.NewRemoteClient(
		Builtin(teleport.RoleAuth), s.server.Addr(), certPool)
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestAcceptedUsage(c *check.C) {
	srv, err := NewAuthServer(AuthServerConfig{
		Dir:           c.MkDir(),
		ClusterName:   "remote",
	...
	user, _, err := test.CreateUserAndRole(srv.AuthServer, "user", []string{"role"})
	c.Assert(err, check.IsNil)

	tlsServer, err := srv.NewTLSServer()
	c.Assert(err, check.IsNil)
	defer tlsServer.Close()

	// Unrestricted clients can use restricted servers
	client, err := tlsServer.NewClient(User(user.GetName()))
	c.Assert(err, check.IsNil)

	// certificate authority is not recognized, because
	// the trust has not been established yet
	_, err = client.GetNodes(defaults.Namespace, resource.SkipValidation())
	c.Assert(err, check.IsNil)

	// restricted clients can use restricted servers if restrictions
	// exactly match
	identity := User(user.GetName())
	...
	// restricted clients will be rejected if usage does not match
	identity = User(user.GetName())
...
func (s *TLSSuite) TestRemoteRotation(c *check.C) {
	...
	err = remoteServer.AuthServer.RotateCertAuthority(server.RotateRequest{
	...
	_, err = newRemoteProxy.GetNodes(defaults.Namespace, resource.SkipValidation())
	c.Assert(err, check.IsNil)

	// old proxy client is still trusted
	_, err = s.server.CloneClient(remoteProxy).GetNodes(defaults.Namespace, resource.SkipValidation())
	c.Assert(err, check.IsNil)
}
...
func (s *TLSSuite) TestLocalProxyPermissions(c *check.C) {
	...
	proxy, err := s.server.NewClient(Builtin(teleport.RoleProxy))
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestAutoRotation(c *check.C) {
	...
	_, err = proxy.GetNodes(defaults.Namespace, resource.SkipValidation())
	c.Assert(err, check.IsNil)
	...
	_, err = s.server.CloneClient(proxy).GetNodes(defaults.Namespace, resource.SkipValidation())
	c.Assert(err, check.IsNil)

	// new clients work as well
	_, err = s.server.NewClient(Builtin(teleport.RoleProxy))
	c.Assert(err, check.IsNil)
	...
	// new clients work as well
	newProxy, err := s.server.NewClient(Builtin(teleport.RoleProxy))
	c.Assert(err, check.IsNil)

	_, err = newProxy.GetNodes(defaults.Namespace, resource.SkipValidation())
	c.Assert(err, check.IsNil)
	...
	// new clients work
	_, err = s.server.CloneClient(newProxy).GetNodes(defaults.Namespace, resource.SkipValidation())
	c.Assert(err, check.IsNil)
}
...
func (s *TLSSuite) TestAppTokenRotation(c *check.C) {
	client, err := s.server.NewClient(Builtin(teleport.RoleApp))
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestRemoteUser(c *check.C) {
	...
	remoteUser, remoteRole, err := test.CreateUserAndRole(remoteServer.AuthServer, "remote-user", []string{"remote-role"})
	c.Assert(err, check.IsNil)
	...
	remoteClient, err := remoteServer.NewRemoteClient(
		User(remoteUser.GetName()), s.server.Addr(), certPool)
	c.Assert(err, check.IsNil)
	...
	_, localRole, err := test.CreateUserAndRole(s.server.Auth(), "local-user", []string{"local-role"})
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestNopUser(c *check.C) {
	client, err := s.server.NewClient(Nop())
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestReadOwnRole(c *check.C) {
	...
	user1, userRole, err := test.CreateUserAndRoleWithoutRoles(clt, "user1", []string{"user1"})
	c.Assert(err, check.IsNil)

	user2, _, err := test.CreateUserAndRoleWithoutRoles(clt, "user2", []string{"user2"})
	c.Assert(err, check.IsNil)

	// user should be able to read their own roles
	userClient, err := s.server.NewClient(User(user1.GetName()))
	c.Assert(err, check.IsNil)
	...
	userClient2, err := s.server.NewClient(Identity{I: server.LocalUser{Username: user2.GetName()}})
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestAppServerCRUD(c *check.C) {
	clt, err := s.server.NewClient(Builtin(teleport.RoleApp))
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestTokens(c *check.C) {
	...
	out, err := clt.GenerateToken(ctx, server.GenerateTokenRequest{Roles: teleport.Roles{teleport.RoleNode}})
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestValidateUploadSessionRecording(c *check.C) {
	...
	for _, tt := range tests {
		clt, err := s.server.NewClient(ServerID(teleport.RoleNode, serverID))
		c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestWebSessionWithoutAccessRequest(c *check.C) {
	...
	_, _, err = test.CreateUserAndRole(clt, user, []string{user})
	c.Assert(err, check.IsNil)

	proxy, err := s.server.NewClient(Builtin(teleport.RoleProxy))
	c.Assert(err, check.IsNil)

	req := server.AuthenticateUserRequest{
		Username: user,
		Pass: &server.PassCreds{
			Password: pass,
		},
	}
...
func (s *TLSSuite) TestWebSessionWithApprovedAccessRequest(c *check.C) {
	...
	newUser, err := test.CreateUserRoleAndRequestable(clt, user, "test-request-role")
	c.Assert(err, check.IsNil)
	...
	accessReq, err := auth.NewAccessRequest(user, "test-request-role")
	c.Assert(err, check.IsNil)
	...
	roles, _, err := resource.ExtractFromCertificate(clt, sshcert)
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestGetCertAuthority(c *check.C) {
	...
	nodeClt, err := s.server.NewClient(Identity{I: server.BuiltinRole{Username: "00000000-0000-0000-0000-000000000000", Role: teleport.RoleNode}})
	c.Assert(err, check.IsNil)
	defer nodeClt.Close()
	...
	userClt, err := s.server.NewClient(User(user.GetName()))
	c.Assert(err, check.IsNil)
	defer userClt.Close()
...
func (s *TLSSuite) TestAccessRequest(c *check.C) {
	...
	user := "user1"
	role := "some-role"
	_, err = test.CreateUserRoleAndRequestable(s.server.Auth(), user, role)
	c.Assert(err, check.IsNil)
	...
	user2, _, err := test.CreateUserAndRole(s.server.Auth(), "user2", []string{"user2"})
	c.Assert(err, check.IsNil)

	testUser2 := User(user2.GetName())
	...
	req, err := auth.NewAccessRequest(user, role)
	c.Assert(err, check.IsNil)
	...
	badReq, err := auth.NewAccessRequest(user, "some-fake-role")
	c.Assert(err, check.IsNil)
...
func TestGenerateCerts(t *testing.T) {
	...
	hostClient, err := srv.NewClient(Identity{I: server.BuiltinRole{Username: hostID, Role: teleport.RoleNode}})
	require.NoError(t, err)

	certs, err := hostClient.GenerateServerKeys(
		server.GenerateServerKeysRequest{
			HostID:               hostID,
	...
	_, err = hostClient.GenerateServerKeys(server.GenerateServerKeysRequest{
	...
	user1, userRole, err := test.CreateUserAndRole(srv.Auth(), "user1", []string{"user1"})
	require.NoError(t, err)

	user2, userRole2, err := test.CreateUserAndRole(srv.Auth(), "user2", []string{"user2"})
	require.NoError(t, err)

	// unauthenticated client should NOT be able to generate a user cert without auth
	nopClient, err := srv.NewClient(Nop())
	require.NoError(t, err)
	...
	adminClient, err := srv.NewClient(Admin())
	require.NoError(t, err)
...
func (s *TLSSuite) TestGenerateAppToken(c *check.C) {
	authClient, err := s.server.NewClient(Builtin(teleport.RoleAdmin))
	c.Assert(err, check.IsNil)
	...
	key, err := auth.GetJWTSigner(ca, s.clock)
	c.Assert(err, check.IsNil)
	...
	for _, tt := range tests {
		client, err := s.server.NewClient(Builtin(tt.inMachineRole))
...
func (s *TLSSuite) TestCertificateFormat(c *check.C) {
	...
	user, userRole, err := test.CreateUserAndRole(s.server.Auth(), "user", []string{"user"})
	c.Assert(err, check.IsNil)
	...
	for _, tt := range tests {
		...
		proxyClient, err := s.server.NewClient(Builtin(teleport.RoleProxy))
		c.Assert(err, check.IsNil)

		// authentication attempt fails with password auth only
		re, err := proxyClient.AuthenticateSSHUser(server.AuthenticateSSHRequest{
			AuthenticateUserRequest: server.AuthenticateUserRequest{
...
func (s *TLSSuite) TestAuthenticateWebUserOTP(c *check.C) {
	...
	_, err = proxy.AuthenticateWebUser(server.AuthenticateUserRequest{
		Username: user,
		OTP:      &server.OTPCreds{Password: []byte("wrong123"), Token: validToken},
	})
	fixtures.ExpectAccessDenied(c, err)
	...
	_, err = proxy.AuthenticateWebUser(server.AuthenticateUserRequest{
		Username: user,
		OTP:      &server.OTPCreds{Password: pass, Token: "wrong123"},
	})
	fixtures.ExpectAccessDenied(c, err)
	...
	ws, err := proxy.AuthenticateWebUser(server.AuthenticateUserRequest{
		Username: user,
		OTP:      &server.OTPCreds{Password: pass, Token: validToken},
	})
	c.Assert(err, check.IsNil)
...
func (s *TLSSuite) TestChangePasswordWithToken(c *check.C) {
	...
	_, _, err = test.CreateUserAndRole(clt, username, []string{"role1"})
	c.Assert(err, check.IsNil)

	token, err := s.server.Auth().CreateResetPasswordToken(context.TODO(), server.CreateResetPasswordTokenRequest{
		Name: username,
		TTL:  time.Hour,
	})
	c.Assert(err, check.IsNil)
	...
	_, err = s.server.Auth().ChangePasswordWithToken(context.TODO(), server.ChangePasswordWithTokenRequest{
...
func (s *TLSSuite) TestLoginNoLocalAuth(c *check.C) {
	...
	_, err = s.server.Auth().AuthenticateWebUser(server.AuthenticateUserRequest{
		Username: user,
		Pass: &server.PassCreds{
			Password: pass,
		},
	...
	_, pub, err := s.server.Auth().GenerateKeyPair("")
	c.Assert(err, check.IsNil)
	_, err = s.server.Auth().AuthenticateSSHUser(server.AuthenticateSSHRequest{
		AuthenticateUserRequest: server.AuthenticateUserRequest{
			Username: user,
			Pass: &server.PassCreds{
				Password: pass,
...
func (s *TLSSuite) TestCipherSuites(c *check.C) {
	otherServer, err := s.server.AuthServer.NewTLSServer()
	c.Assert(err, check.IsNil)
	defer otherServer.Close()

	// Create a client with ciphersuites that the server does not support.
	tlsConfig, err := s.server.ClientTLSConfig(Nop())
	c.Assert(err, check.IsNil)
	...
	client, err := client.New(apiclient.Config{
		Addrs: addrs,
...
func (s *TLSSuite) TestRegisterCAPin(c *check.C) {
	...
	token, err := s.server.AuthServer.AuthServer.GenerateToken(ctx, server.GenerateTokenRequest{
	...
	_, err = client.Register(client.RegisterParams{
...
func (s *TLSSuite) TestEventsNodePresence(c *check.C) {
	...
	clt, err := s.server.NewClient(Identity{
	...
	nopClt, err := s.server.NewClient(Builtin(teleport.RoleNop))
	c.Assert(err, check.IsNil)
	defer nopClt.Close()
...
func (s *TLSSuite) TestEventsPermissions(c *check.C) {
	clt, err := s.server.NewClient(Builtin(teleport.RoleNode))
	c.Assert(err, check.IsNil)
	defer clt.Close()
	...
	test.ExpectResource(c, w, 3*time.Second, ca)
	...
	testCases := []testCase{
		{
			...
			identity: Builtin(teleport.RoleNode),
		...
		},
		{
			...
			identity: Builtin(teleport.RoleNop),
			watches: []services.WatchKind{
...
func (s *TLSSuite) TestEventsClusterConfig(c *check.C) {
	clt, err := s.server.NewClient(Builtin(teleport.RoleAdmin))
	c.Assert(err, check.IsNil)
	defer clt.Close()
	...
	test.ExpectResource(c, w, 3*time.Second, staticTokens)
	...
	test.ExpectResource(c, w, 3*time.Second, token)

	// delete token and expect delete event
	err = s.server.Auth().DeleteToken(token.GetName())
	c.Assert(err, check.IsNil)
	test.ExpectDeleteResource(c, w, 3*time.Second, &services.ResourceHeader{
		Kind:    services.KindToken,
		Version: services.V2,
		Metadata: services.Metadata{
			Namespace: defaults.Namespace,
	...
	test.ExpectResource(c, w, 3*time.Second, clusterConfig)
	...
	test.ExpectResource(c, w, 3*time.Second, clusterNameResource)
}
...
func newTestTLSServer(t *testing.T) *TLSServer {
