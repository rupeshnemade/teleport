...
func (s *ServicesTestSuite) TestUsersCRUD(c *check.C) {
	...
	u, err = s.WebS.GetUsers(false)
	c.Assert(err, check.IsNil)
	userSlicesEqual(c, u, []types.User{newUser("user1"), newUser("user2")})
	...
	u, err = s.WebS.GetUsers(false)
	c.Assert(err, check.IsNil)
	userSlicesEqual(c, u, []types.User{newUser("user2")})
...
func (s *ServicesTestSuite) TestTokenCRUD(c *check.C) {
	...
	data, err := resource.MarshalProvisionToken(v2, resource.WithVersion(types.V1))
	c.Assert(err, check.IsNil)
...
func (s *ServicesTestSuite) TestGithubConnectorCRUD(c *check.C) {
	connector := &types.SAMLConnectorV2{
		Kind:    types.KindSAML,
		Version: types.V2,
		Metadata: types.Metadata{
...
func (s *ServicesTestSuite) TestSemaphoreFlakiness(c *check.C) {
	const renewals = 3
	// wrap our types.Semaphores instance to cause two out of three lease
...
func (s *ServicesTestSuite) TestProxyWatcher(c *check.C) {
	...
	proxy := suite.NewServer(types.KindProxy, "proxy1", "127.0.0.1:2023", defaults.Namespace)
	...
	// add a second proxy
	proxy2 := suite.NewServer(types.KindProxy, "proxy2", "127.0.0.1:2023", defaults.Namespace)
