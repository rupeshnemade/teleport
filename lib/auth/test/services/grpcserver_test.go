...
func TestMFADeviceManagement(t *testing.T) {
	...
	// Create a fake user.
	user, _, err := test.CreateUserAndRole(srv.Auth(), "mfa-user", []string{"role"})
	require.NoError(t, err)
	cl, err := srv.NewClient(User(user.GetName()))
	require.NoError(t, err)
	...
	}{
		{
			...
			opts: mfaAddTestOpts{
				...
				wantDev: func(t *testing.T) *types.MFADevice {
					wantDev, err := auth.NewTOTPDevice("totp-dev", totpSecrets["totp-dev"], clock.Now())
					require.NoError(t, err)
					return wantDev
				},
			},
...
func testAddMFADevice(ctx context.Context, t *testing.T, cl *client.Client, opts mfaAddTestOpts) {
...
func testDeleteMFADevice(ctx context.Context, t *testing.T, cl *client.Client, opts mfaDeleteTestOpts) {
...
func testGenerateUserSingleUseCert(ctx context.Context, t *testing.T, cl *client.Client, opts generateUserSingleUseCertTestOpts) {
