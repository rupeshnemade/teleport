...
func (s *Server) ChangePassword(req auth.ChangePasswordReq) error {
	ctx := context.TODO()
	// validate new password
	if err := auth.VerifyPassword(req.NewPassword); err != nil {
		return trace.Wrap(err)
...
func (s *Server) changePasswordWithToken(ctx context.Context, req ChangePasswordWithTokenRequest) (services.User, error) {
	...
	err = auth.VerifyPassword(req.Password)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func (s *Server) changeUserSecondFactor(req ChangePasswordWithTokenRequest, token services.ResetPasswordToken) error {
	...
	if req.SecondFactorToken != "" {
		...
		dev, err := auth.NewTOTPDevice("otp", secrets.GetOTPKey(), s.clock.Now())
		if err != nil {
			return trace.Wrap(err)
		}
