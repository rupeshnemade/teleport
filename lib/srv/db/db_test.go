...
func TestDatabaseAccessDisabled(t *testing.T) {
	...
	// Create user/role with the requested permissions.
	_, role, err := test.CreateUserAndRole(testCtx.tlsServer.Auth(), userName, []string{roleName})
	require.NoError(t, err)
...
func setupTestContext(ctx context.Context, t *testing.T) *testContext {
	...
	require.NoError(t, err)
	dbAuthorizer, err := server.NewAuthorizer(clusterName, dbAuthClient, dbAuthClient, dbAuthClient)
	require.NoError(t, err)

	// Auth client/authorizer for database proxy.
	proxyAuthClient, err := tlsServer.NewClient(testservices.Builtin(teleport.RoleProxy))
	require.NoError(t, err)
	proxyAuthorizer, err := server.NewAuthorizer(clusterName, proxyAuthClient, proxyAuthClient, proxyAuthClient)
	require.NoError(t, err)
